...
一个交互式 CLI 学习平台，帮助你快速掌握 Claude Code 的 118 个命令。包含交互终端模拟、情景挑战和成就系统三大模块，让 CLI 学习像玩游戏一样有趣。
...
### 命令分类系统
118 个命令按功能分类，支持按类别筛选和学习进度追踪。

### 交互终端模拟
在浏览器中模拟真实的 CLI 环境，支持命令输入、输出解析和错误提示。

### 情景挑战
预设真实开发场景，通过完成任务来检验学习成果。

### 成就系统
内置成就引擎，记录学习进度，激励持续学习。

## 开发心得

在设计 CLI 模拟器时，最大的挑战是如何在 Web 环境中真实还原终端体验。选择了 xterm.js 作为基础，在其上封装了命令解析和执行引擎，同时加入了智能提示和自动补全功能。
